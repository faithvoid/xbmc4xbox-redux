...
bool CHomeRunDirectory::GetDirectory(const CURL& url, CFileItemList &items)
{
  ...
  if(url.GetHostName().IsEmpty())
  {
    ...
    int count = m_pdll->discover_find_devices(HDHOMERUN_DEVICE_TYPE_TUNER, result_list, 64);
      if (count < 0)
      return false;
  ...
  {    
    hdhomerun_device_t* device = m_pdll->device_create_from_str(url.GetHostName().c_str());
    if(!device)
      return false;
