  ...
  try
  {
    ...
    if (item.HasVideoInfoTag() && item.GetVideoInfoTag()->m_iDbId > 0)
    {
      ...
      if (item.GetVideoInfoTag()->m_playCount != count)
      {
        ...
        ANNOUNCEMENT::CAnnouncementManager::GetInstance().Announce(ANNOUNCEMENT::VideoLibrary, "xbmc", "OnUpdate", CFileItemPtr(new CFileItem(item)));
    }
    ...
    if (CProfilesManager::Get().GetMasterProfile().getLockMode() != LOCK_MODE_EVERYONE && !g_passwordManager.bMasterUser)
    {
      ...
      for (it=mapSeasons.begin();it != mapSeasons.end();++it)
      {
        ...
        if (iSeason == 0)
          ...
          strLabel.Format(g_localizeStrings.Get(20358).c_str(),iSeason);
        CFileItemPtr pItem(new CFileItem(strLabel));
    ...
    case VIDEODB_CONTENT_TVSHOWS:
    {
      ...
      while (i < items.Size())
      {
        ...
        while (j < items.Size())
        {
          ...
          if (jItem->GetVideoInfoTag()->m_strTitle == strTitle)
          {
