...
bool CGUIDialogSubtitles::SetService(const std::string &service)
{
  if (service != m_currentService)
  {
    ...
    if (image)
      image->SetFileName(currentService->GetThumbnailImage());
  ...
  switch (status)
  {
    ...
    case SEARCH_COMPLETE:
      if (!m_subtitles->IsEmpty())
        label = StringUtils2::Format(g_localizeStrings.Get(24108).c_str(), m_subtitles->Size());
      else
...
//    CLog::Log(LOGDEBUG, "Stack Subs [%s} Found", vecItems[i]->GetLabel().c_str());
  ...
  else if (StringUtils2::StartsWith(g_application.CurrentFile(), "http://"))
  {
  ...
  if (strSubExt.Equals(".sub"))
  {
    ...
    {
      CStdString strSubNameIdx = StringUtils2::Format("%s.%s.idx", strFileName.c_str(), strSubLang.c_str());
