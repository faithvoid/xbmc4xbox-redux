...
string ByPath(SortAttribute attributes, const SortItem &values)
{
  ...
  CStdString label;
  label.Format("%s %d", values.find(FieldPath)->second.asString().c_str(), startOffset);
  return label;
}
...
{
  string title = values.find(FieldTitle)->second.asString();
  if (attributes & SortAttributeIgnoreArticle)
...
string ByEpisodeNumber(SortAttribute attributes, const SortItem &values)
{
  ...
  if (itEpisodeSpecial != values.end() && itSeasonSpecial != values.end() &&
    ...
    num = ((uint64_t)itSeasonSpecial->second.asInteger() << 32) + (itEpisodeSpecial->second.asInteger() << 16) - ((2 << 15) - values.find(FieldEpisodeNumber)->second.asInteger());
  else
