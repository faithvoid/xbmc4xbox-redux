...
// because the string match is performed with StringUtils2::StartsWith()
...
std::string CLegacyPathTranslation::TranslatePath(const std::string &legacyPath, Translator *translationMap, size_t translationMapSize)
{
  ...
  {
    if (StringUtils2::StartsWith(newPath, translationMap[index].legacyPath))
    {
      StringUtils2::Replace(newPath, translationMap[index].legacyPath, translationMap[index].newPath);
      break;
    }
  }
