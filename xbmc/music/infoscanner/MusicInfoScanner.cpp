...
bool CMusicInfoScanner::DoScan(const CStdString& strDirectory)
{
  ...
  // load subfolder
  CFileItemList items;
  CDirectory::GetDirectory(strDirectory, items, g_settings.m_musicExtensions + "|.jpg|.tbn");
...
void CMusicInfoScanner::CheckForVariousArtists(VECSONGS &songsToCheck)
{
  ...
  for (unsigned int i = 0; i < songsToCheck.size(); ++i)
  {
    ...
    if (!song.albumArtist.empty()) // albumartist specified, so assume the user knows what they're doingg
      continue;
