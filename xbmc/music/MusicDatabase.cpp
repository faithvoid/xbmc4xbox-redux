...
void CMusicDatabase::GetFileItemFromDataset(const dbiplus::sql_record* const record, CFileItem* item, const CStdString& strMusicDBbasePath)
{
  ...
  item->GetMusicInfoTag()->SetDatabaseId(record->at(song_idSong).get_asInt(), "id");
  SYSTEMTIME stTime;
...
bool CMusicDatabase::GetRecentlyAddedAlbumSongs(const CStdString& strBaseDir, CFileItemList& items, unsigned int limit)
{
  try
  {
    ...
    CStdString strSQL;
    strSQL.Format("SELECT songview.* FROM (SELECT idAlbum FROM albumview ORDER BY idAlbum DESC LIMIT %u) AS recentalbums JOIN songview ON songview.idAlbum=recentalbums.idAlbum", limit ? limit : g_advancedSettings.m_iMusicLibraryRecentlyAddedItems);
...
bool CMusicDatabase::GetAlbumsNav(const CStdString& strBaseDir, CFileItemList& items, int idGenre /* = -1 */, int idArtist /* = -1 */, const SortDescription &sortDescription /* = SortDescription() */)
{
  ...
  Filter filter;
  bool bResult = GetAlbumsByWhere(musicUrl.ToString(), filter, items, sortDescription);
...
bool CMusicDatabase::GetSongsByWhere(const CStdString &baseDir, const Filter &filter, CFileItemList &items, const SortDescription &sortDescription /* = SortDescription() */)
{
  ...
  try
  {
    ...
    // cleanup
    m_pDS->close();
    CLog::Log(LOGDEBUG, "%s(%s) - took %d ms", __FUNCTION__, filter.where.c_str(), timeGetTime() - time);
    return true;
  }
  catch (...)
  {
...
bool CMusicDatabase::UpdateOldVersion(int version)
{
  ...
  try
  {
      ...
      if (m_pDS->query("select * from thumb where strThumb != 'NONE'") && m_pDS->num_rows())
      {
        ...
        while (!m_pDS->eof())
        {
          ...
          if (thumb.Left(oldPath.size()).CompareNoCase(oldPath) == 0)
          {
            ...
            CStdString sql = PrepareSQL("update thumb set strThumb='%s' where idThumb=%i\n", thumb.c_str(), id);
    ...
    if (version < 14)
    {
      ...
      for (int i=0;i<2;++i)
      {
        CStdString strSQL=PrepareSQL("select %s,strImage from %s",
                                    tag1[i],tag2[i]);
        m_pDS->query(strSQL.c_str());
        while (!m_pDS->eof())
        {
          ...
          CStdString strSQL = PrepareSQL("update %s set strImage='%s' where %s=%i",
                                        tag2[i],
